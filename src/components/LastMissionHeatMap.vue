<template>
  <div>
    ...
    <div id="last-mission-heat-map"></div>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    const statusMaster = [
      {id: 'not-started', name: '未クリア', min: 0, max: 0, color: 'white' },
      {id: 'first-quarter', name: 'クリア', min: 1, max: 25, color: 'rgb(222,237,250)' },
      {id: 'second-quarter', name: '銅%', min: 26, max: 50, color: 'rgb(190,218,246)' },
      {id: 'third-quarter', name: '銀', min: 51, max: 75, color: 'rgb(142,191,239)' },
      {id: 'fourth-quarter', name: '金', min: 76, max: 99, color: 'rgb(125,182,236)' },
    ]
    return {
      highChart: null,
  ...
  methods: {
    ...
    createHeatMap(users) {
      ...
      return Highcharts.chart('last-mission-heat-map', {
        chart: {
          type: 'heatmap',
          scrollablePlotArea: {
            minHeight,
